...
import MovieTheaterForm from "../movietheaters/MovieTheaterForm";
...
export default function MovieDetails(){
    ...
    return(
            movie ? <div>
                ...
                {movie.genres?.map(genre => <Link key={genre.id} style={{marginRight: '5px'}}
                className="btn btn-primary btn-sm rounded-pill" to={`/movies/filter?genreId=${genre.id}`}>{genre.name}</Link>)}
                | {movie.releaseDate?.toDateString()}
            
            <div style={{display: 'flex', marginTop: '1rem'}}>
                <span style={{display: 'inline-block', marginRight: '1rem'}}>
                    <img src={movie.poster}
                    style={{width: '225px', height: '315px'}} alt="poster"></img>
                </span>
                {movie.trailer ? <div>
                    <iframe title="youtube-trailer" width="560" height="315" src={generateEmbeddedVideoURL(movie.trailer)}
            ...
            <div style={{marginTop: '1rem'}}>
                ...
                <div style={{display: 'flex', flexDirection: 'column'}}>
                    {movie.actors.map(actor => <div key={actor.id} style={{marginBottom: '2px'}}>
